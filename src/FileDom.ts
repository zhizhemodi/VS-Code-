...
export class FileDom {
    ...
    constructor(imagePath: string, opacity: number) {
        ...
        this.imageOpacity = this.imageOpacity;
    }
    ...
    public imageToBase64() {
        try {
            ...
            let dirname = path.dirname(this.imagePath);
            if(dirname.startsWith('..')){
                this.imagePath = fs.readFileSync(path.resolve(__dirname, this.imagePath)).toString('base64');
            }
            else {
